...
public class ContainerTank extends ContainerBase {
	...
	public ContainerTank(BlockPos pos, InventoryPlayer inv, TileEntityTank tank) {
		...
		addSlotToContainer(new SlotOxygenHandler(handler, 0, 44, 38));
		addSlotToContainer(new SlotOxygenHandler(handler, 1, 116, 38));
		...
		for (int k = 0; k < 9; k++) {
			this.addSlotToContainer(new Slot(inv, k, 8 + k * 18, 146));
		}
	}
